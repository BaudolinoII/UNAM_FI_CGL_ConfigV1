...
int main() {
	...
	while (!glfwWindowShouldClose(window)) {
		...
		model = glm::rotate(model, glm::radians(dedo1), glm::vec3(0.0f, 0.0, 1.0f));
		modelTemp = model = glm::translate(model, glm::vec3(0.625f, 0.0f, 0.0f));
		model = glm::scale(model, glm::vec3(1.25f, 0.3f, 0.25f));
		color = glm::vec3(0.0f, 1.0f, 1.0f);
		glUniform3fv(uniformColor, 1, glm::value_ptr(color));
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
		glDrawArrays(GL_TRIANGLES, 0, 36);

		model = glm::translate(modelTemp, glm::vec3(0.625f, 0.0f, 0.0f));
		model = glm::rotate(model, glm::radians(dedo2), glm::vec3(0.0f, 0.0, 1.0f));
		model = glm::translate(model, glm::vec3(0.5f, 0.0f, 0.0f));
		model = glm::scale(model, glm::vec3(1.0f, 0.3f, 0.25f));
		color = glm::vec3(1.0f, 0.0f, 1.0f);
		...
		//////Pulgar
		model = glm::translate(modelTemp2, glm::vec3(0.125f, -0.35f, 0.375f));
...
void Inputs(GLFWwindow* window) {
	...
	if (glfwGetKey(window, GLFW_KEY_U) == GLFW_PRESS) {
		dedo1 = (dedo1 > 65.0f ? 65.0f : dedo1 + 0.18);
		pulgar = (pulgar > 90.0f ? 90.0f : pulgar + 0.18);
	}
	if (glfwGetKey(window, GLFW_KEY_J) == GLFW_PRESS) {
		dedo1 = (dedo1 < -45.0f ? -45.0f : dedo1 - 0.18);
		pulgar = (pulgar < -60.0f ? -60.0f : pulgar - 0.18);
	}
	if (glfwGetKey(window, GLFW_KEY_I) == GLFW_PRESS)
		dedo2 = (dedo2 > 0.0f ? 0.0f : dedo2 + 0.18);
	if (glfwGetKey(window, GLFW_KEY_K) == GLFW_PRESS)
		dedo2 = (dedo2 < -90.0f ? -90.0f : dedo2 - 0.18);
